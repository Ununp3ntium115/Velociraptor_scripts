...
function New-PackageDirectory {
    ...
    if (Test-Path $script:PackagePath) {
        Remove-Item $script:PackagePath -Recurse -Force -ErrorAction SilentlyContinue
    }

    if (-not $DryRun) {
        New-Item -Path $script:PackagePath -ItemType Directory -Force -ErrorAction SilentlyContinue | Out-Null
    }
...
function New-ReleaseArchives {
    ...
    if (-not $DryRun) {
        Compress-Archive -Path "$script:PackagePath\*" -DestinationPath $zipPath -Force -ErrorAction SilentlyContinue
    }
...
function Publish-ToPowerShellGallery {
    ...
    try {
        ...
        Publish-Module -Path $script:RootPath -NuGetApiKey $PSGalleryApiKey -Verbose -Force -ErrorAction SilentlyContinue
