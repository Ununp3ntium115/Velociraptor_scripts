...
function Deploy-Configuration {
    ...
    if ($ModuleLoaded -and (Get-Command Test-VelociraptorConfiguration -ErrorAction SilentlyContinue)) {
        ...
        if ($validation.SecurityIssues.Count -gt 0) {
            ...
            if ($Environment -eq 'Production' -and -not $Force) {
                throw "Security issues found in Production configuration. Use -Force -ErrorAction SilentlyContinue to override."
            }
        }
    }
    ...
    if (Test-Path $targetConfigPath) {
        ...
        if ($ModuleLoaded -and (Get-Command Backup-VelociraptorConfiguration -ErrorAction SilentlyContinue)) {
            ...
            Copy-Item $targetConfigPath $backupPath -Force -ErrorAction SilentlyContinue
    ...
    if ($ModuleLoaded -and (Get-Command Set-VelociraptorSecurityHardening -ErrorAction SilentlyContinue)) {
        ...
        $hardeningResult = Set-VelociraptorSecurityHardening -ConfigPath $ConfigPath -HardeningLevel $CurrentEnvConfig.settings.security_level -Force -ErrorAction SilentlyContinue:$Force
    ...
    Copy-Item $ConfigPath $targetConfigPath -Force -ErrorAction SilentlyContinue
...
try {
        ...
        if (Test-Path $backupDir) {
            ...
            if ($latestBackup) {
                ...
                if ($ModuleLoaded -and (Get-Command Restore-VelociraptorConfiguration -ErrorAction SilentlyContinue)) {
                    $restoreResult = Restore-VelociraptorConfiguration -BackupPath $latestBackup.FullName -Force -ErrorAction SilentlyContinue:$Force
                    ...
                    Copy-Item $latestBackup.FullName $targetPath -Force -ErrorAction SilentlyContinue
                    Write-DeployLog "Rollback completed successfully" -Level Success
    ...
    if ($ModuleLoaded -and (Get-Command Add-VelociraptorFirewallRule -ErrorAction SilentlyContinue)) {
        ...
        Add-VelociraptorFirewallRule -Port $frontendPort -RuleName "Velociraptor Frontend ($Environment)" -Force -ErrorAction SilentlyContinue
        Add-VelociraptorFirewallRule -Port $guiPort -RuleName "Velociraptor GUI ($Environment)" -Force -ErrorAction SilentlyContinue
    ...
    if ($Environment -eq 'Production' -and -not $Rollback) {
        ...
        try {
            & $MyInvocation.MyCommand.Path -Environment $Environment -Rollback -Force -ErrorAction SilentlyContinue
        }
        catch {
