function Restore-VelociraptorConfiguration {
    ...
    try {
        ...
        if ($isCompressed) {
            ...
            Expand-Archive -Path $BackupPath -DestinationPath $workingDir -Force -ErrorAction SilentlyContinue
        ...
        if ($CreateBackupBeforeRestore -and (Test-Path $RestorePath)) {
            ...
            Copy-Item $RestorePath $preRestoreBackup -Force -ErrorAction SilentlyContinue
        ...
        if (-not (Test-Path $restoreDir)) {
            New-Item -ItemType Directory -Path $restoreDir -Force -ErrorAction SilentlyContinue | Out-Null
        }

        # Restore configuration file
        Copy-Item $configFile $RestorePath -Force -ErrorAction SilentlyContinue
            ...
            if ($datastoreRestorePath) {
                ...
                try {
                    ...
                    if (-not (Test-Path $datastoreRestorePath)) {
                        New-Item -ItemType Directory -Path $datastoreRestorePath -Force -ErrorAction SilentlyContinue | Out-Null
                    }

                    # Copy datastore files
                    Copy-Item -Path "$datastoreDir\*" -Destination $datastoreRestorePath -Recurse -Force -ErrorAction SilentlyContinue
