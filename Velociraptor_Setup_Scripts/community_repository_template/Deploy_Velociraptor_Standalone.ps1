...
.DESCRIPTION
    ...
    Creates data directory for GUI interface
    Adds an inbound firewall rule for TCP 8889 (netsh fallback)
    Launches velociraptor.exe gui --datastore
...
.NOTES
    Velociraptor Setup Scripts (Community Edition) v5.0.1-beta
    Requires Administrator privileges
    Community Edition - Open Source DFIR Deployment
#>
...
Write-Host "  Velociraptor Setup Scripts (Community Edition)    " -ForegroundColor Blue  
Write-Host "            v5.0.1-beta - Standalone Mode           " -ForegroundColor Blue
Write-Host "=====================================================" -ForegroundColor Blue
Write-Host ""
...
function Get-LatestVelociraptorAsset {
    ...
    try {
        $headers = @{ 
            'User-Agent' = 'VelociraptorCommunityDeployer/5.0.1'
            'Accept'     = 'application/vnd.github.v3+json'
        }
        $release = Invoke-RestMethod -Uri 'https://api.github.com/repos/Ununp3ntium115/velociraptor/releases/latest' -Headers $headers -TimeoutSec 30
...
try {
    ...
    Write-Log '==== Velociraptor Community Deployment Started ====' -Level 'Success'
    ...
    Write-Host "  ✅ COMMUNITY DEPLOYMENT SUCCESSFUL                " -ForegroundColor Green
    Write-Host "  📧 Community Edition - Open Source DFIR        " -ForegroundColor Yellow
    Write-Host "=====================================================" -ForegroundColor Green
}
catch {
