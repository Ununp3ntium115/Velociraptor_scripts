...
<#
.SYNOPSIS
    Fixed Live Velociraptor Installation - Downloads and installs the actual executable

.DESCRIPTION
    This script downloads the real Velociraptor executable from GitHub and performs
    a complete installation with fixed variable scoping.
#>
...
Write-Host "FIXED LIVE VELOCIRAPTOR INSTALLATION" -ForegroundColor Cyan
...
$form.Text = "Fixed Live Velociraptor Installation"
...
$header.Text = "FIXED LIVE VELOCIRAPTOR INSTALLATION"
...
$installBtn.Add_Click({
    try {
        ...
        Add-Log "=== STARTING FIXED VELOCIRAPTOR INSTALLATION ==="
        ...
        # Direct GitHub API call (avoiding module dependency issues)
        try {
            Add-Log "Using direct GitHub API call..."
            ...
            if ($windowsAsset) {
                ...
                Add-Log "SUCCESS - Found Velociraptor v$($release.Version)"
        ...
        try {
            Add-Log "Starting download using Invoke-WebRequest..."
            
            # Use Invoke-WebRequest with progress
            $webRequest = Invoke-WebRequest -Uri $release.Asset.DownloadUrl -OutFile $exePath -PassThru
            
            if (Test-Path $exePath) {
                $fileSize = (Get-Item $exePath).Length
                Add-Log "Download completed successfully"
                Add-Log "  File size: $([math]::Round($fileSize / 1MB, 2)) MB"
                Add-Log "  Expected: $([math]::Round($release.Asset.Size / 1MB, 2)) MB"
                
                if ([math]::Abs($fileSize - $release.Asset.Size) -lt 1024) {
                    Add-Log "File size verification: PASSED"
                } else {
                    Add-Log "File size verification: WARNING - Size mismatch"
                }
            } else {
        ...
        try {
            ...
            if (Test-Path $exePath) {
                ...
                Add-Log "File exists: $($fileInfo.Name) ($([math]::Round($fileInfo.Length / 1MB, 2)) MB)"
                
                # Try to get version info
                $versionOutput = & $exePath version 2>&1
                if ($LASTEXITCODE -eq 0) {
                    Add-Log "Executable verification successful"
                    Add-Log "  Version: $($versionOutput | Select-Object -First 1)"
                } else {
                    Add-Log "Executable verification completed with exit code $LASTEXITCODE"
                    Add-Log "  Output: $versionOutput"
                }
            } else {
            ...
            Add-Log "Continuing with installation anyway..."
        }
...
"@
        ...
        Add-Log "Server configuration created successfully"
        ...
        try {
            if ($isAdmin) {
                ...
                $result1 = netsh advfirewall firewall add rule name="Velociraptor GUI" dir=in action=allow protocol=TCP localport=8889 2>&1
                $result2 = netsh advfirewall firewall add rule name="Velociraptor Frontend" dir=in action=allow protocol=TCP localport=8000 2>&1
...
"@
        ...
        # Create datastore and logs directories
        if (-not (Test-Path "$installPath\datastore")) {
            New-Item -Path "$installPath\datastore" -ItemType Directory -Force | Out-Null
        }
        if (-not (Test-Path "$installPath\filestore")) {
            New-Item -Path "$installPath\filestore" -ItemType Directory -Force | Out-Null
        }
        if (-not (Test-Path "$installPath\logs")) {
            New-Item -Path "$installPath\logs" -ItemType Directory -Force | Out-Null
        }
        Add-Log "Created supporting directories"
        ...
        Add-Log "1. Click 'LAUNCH VELOCIRAPTOR NOW' below"
        Add-Log "2. Or double-click: $launchScript"
        Add-Log "3. Or run manually: $exePath --config '$configPath' gui"
        Add-Log "4. Then open browser to: http://127.0.0.1:8889"
        Add-Log ""
        Add-Log "*** READY TO LAUNCH THE ACTUAL VELOCIRAPTOR! ***"
        ...
        $installBtn.Text = "RETRY INSTALLATION"
        $installBtn.BackColor = [System.Drawing.Color]::FromArgb(244, 67, 54)
        $installBtn.Enabled = $true
    }
})

# Add launch handler (separate event to avoid conflicts)
$launchHandler = {
    try {
        Add-Log "Launching Velociraptor server..."
        $configPath = "C:\VelociraptorData\server.config.yaml"
        $exePath = "C:\VelociraptorData\bin\velociraptor.exe"
        
        if (Test-Path $exePath) {
            Add-Log "Starting Velociraptor with config: $configPath"
            $process = Start-Process -FilePath $exePath -ArgumentList "--config", $configPath, "gui" -WorkingDirectory "C:\VelociraptorData" -PassThru
            Add-Log "Velociraptor process started with PID: $($process.Id)"
            Add-Log "Waiting 8 seconds for server to start..."
            
            # Wait for server to start
            Start-Sleep -Seconds 8
            
            Add-Log "Opening browser to http://127.0.0.1:8889"
            Start-Process "http://127.0.0.1:8889"
            
            Add-Log ""
            Add-Log "=== VELOCIRAPTOR IS NOW RUNNING! ==="
            Add-Log "Access the web interface at: http://127.0.0.1:8889"
            Add-Log "The server is running in the background."
            Add-Log ""
        } else {
            Add-Log "ERROR: Velociraptor executable not found at $exePath"
        }
    }
    catch {
        Add-Log "Failed to launch Velociraptor: $($_.Exception.Message)"
    }
}

Write-Host "Opening Fixed Live Installation GUI..." -ForegroundColor Green
