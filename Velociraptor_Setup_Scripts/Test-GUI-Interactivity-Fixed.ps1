...
.DESCRIPTION
    ...
    - Control enabled states and tab navigation
...
.EXAMPLE
    .\Test-GUI-Interactivity-Fixed.ps1
#>
...
function Test-ControlInteractivity {
    ...
    if ($content -match "\.Enabled\s*=\s*\$true" -or $content -match "\.ReadOnly\s*=\s*\$false") {
    ...
    $eventPatterns = @("Add_Click", "Add_SelectedIndexChanged", "Add_CheckedChanged", "Add_GotFocus", "Add_LostFocus")
    $eventCount = 0
    ...
    if ($content -match "\.Size.*New-Object.*Drawing\.Size.*2\d+|3\d+") {
    ...
    if ($content -match "BackColor.*Focus|MouseEnter|MouseLeave|Cursor.*Hand") {
    ...
    if ($content -match "UseSystemPasswordChar|PasswordChar") {
    ...
    if ($content -match "KeyDown|KeyPress") {
...
function Test-AccessibilityFeatures {
    ...
    if ($content -match "Font.*Size.*1[0-9]|Font.*Size.*[2-9][0-9]") {
    ...
    if ($content -match "Location.*Point.*[2-9][0-9]") {
...
function Test-SpecificControlTypes {
    ...
    Write-Host "`n  📊 Control Type Score: $controlScore/$maxControlScore ($controlPercentage percent)" -ForegroundColor $(if ($controlPercentage -ge 80) { "Green" } elseif ($controlPercentage -ge 60) { "Yellow" } else { "Red" })
