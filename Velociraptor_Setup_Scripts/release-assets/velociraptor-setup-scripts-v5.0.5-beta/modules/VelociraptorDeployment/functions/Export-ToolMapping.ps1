function Export-ToolMapping {
    ...
    try {
        ...
        # Handle artifact counting with comprehensive null checks and safe Count access
        if ($artifactList -and $artifactList -ne $null) {
            try {
                # Use PowerShell's safe counting method - @($var).Count always works
                $artifactArray = @($artifactList)
                $artifactCount = $artifactArray.Count
                $artifactList = $artifactArray

                # Count artifacts with and without tools using safe Count access
        ...
        # Handle tool counting with safe Count access
        if ($toolDatabase -and $toolDatabase -ne $null) {
            try {
                if ($toolDatabase -is [hashtable] -or $toolDatabase -is [System.Collections.IDictionary]) {
                    if ($toolDatabase.Keys) {
                    ...
                    # Fallback: try to get count safely
        ...
        foreach ($artifact in $artifactList) {
            try {
                ...
                # Safe tool list extraction with guaranteed Count access
                try {
                    if ($artifact.Tools) { 
                ...
                # Safe count using array conversion - always works
                $toolArray = @($toolList)
                $toolListCount = $toolArray.Count
                $toolList = $toolArray
            ...
            try {
                ...
                foreach ($toolName in $keys) {
                    try {
                        ...
                        # Safe count using array conversion - guaranteed to work
                        $usedByCount = 0
                        try {
