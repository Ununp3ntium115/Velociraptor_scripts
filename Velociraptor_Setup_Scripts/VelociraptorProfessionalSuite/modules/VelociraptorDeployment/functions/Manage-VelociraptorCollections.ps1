...
function New-CollectionManager {
    ...
    if (-not (Test-Path $manager.DownloadCache)) {
        New-Item -Path $manager.DownloadCache -ItemType Directory -Force | Out-Null
    }
...
function Get-ToolDependency {
    ...
    try {
        ...
        switch ($extension) {
            '.zip' {
                Expand-Archive -Path $tempFile -DestinationPath $downloadPath -Force
            }
            '.tar' {
                ...
                foreach ($dep in $collection.Dependencies) {
                    ...
                    if ($depStatus.Status -in @('Available', 'Cached')) {
                        ...
                        if (-not (Test-Path $toolDestPath)) {
                            New-Item -Path $toolDestPath -ItemType Directory -Force | Out-Null
                        }
                        
                        if ($depStatus.Status -eq 'Available') {
                            Copy-Item $depStatus.Path $toolDestPath -Force
                        }
                        else {
                            Copy-Item "$($depStatus.Path)\*" $toolDestPath -Recurse -Force
                        }
